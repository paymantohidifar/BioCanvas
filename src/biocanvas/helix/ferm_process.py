...
class _GeneralProcessTable:
    ...
    def _compute_gas_metrics(
        df: pd.DataFrame, outlet_n2_col: str, outlet_o2_col: str, outlet_co2_col: str
    ) -> pd.DataFrame:
        ...
        """
        inlet_n2_frac: float = AIR_N2 / (100 - AIR_O2) * (100.0 - df["GasMix"])  # type: ignore
        inlet_co2_frac: float = AIR_CO2 / (100 - AIR_O2) * (100.0 - df["GasMix"])  # type: ignore

        # Exhaust molar flow rate via inert N2 balance
        df["Exhust Flow Rate (mmol/h)"] = (
            df["Inlet Flow Rate (mmol/h)"]
            * (inlet_n2_frac / df.get(outlet_n2_col, np.nan))  # type: ignore
        )
        # OUR — O2 uptake rate
        df["OUR (mmol/h)"] = (
        ...
        df["OURT (g)"] = (df["OUR (mmol/h)"] * dt_h).cumsum().fillna(0) * O2_MW / 1000.0  # type: ignore
        # df['OURT (g)'] = (
        ...
        df["CERT (g)"] = (
            (df["CER (mmol/h)"] * dt_h).cumsum().fillna(0) * CO2_MW / 1000.0
        )  # type: ignore
        # df['CERT (g)'] = (
...
class EveTable(_GeneralProcessTable):
    ...
    def process(self) -> pd.DataFrame:
        ...
        df["Time (h)"] = np.round(  # type: ignore
            df["Batch Time (since inoc.), sec"].values / 3600.0,
            3,  # type: ignore
        )

        # Convert flow units from l/min to ml/min where needed
        df["GM Flow, ml/min"] = df.get(
            "GM Flow, ml/min", df.get("GM Flow, l/min", np.nan) * 1000
        )  # type: ignore
        df["Air Flow, ml/min"] = df.get(
            "Air Flow, ml/min", df.get("Air Flow, l/min", np.nan) * 1000
        )  # type: ignore
        df["O₂ Flow, ml/min"] = df.get(
            "O₂ Flow, ml/min", df.get("O₂ Flow, l/min", np.nan) * 1000
        )  # type: ignore
        df["N₂ Flow, ml/min"] = df.get(
            "N₂ Flow, ml/min", df.get("N₂ Flow, l/min", np.nan) * 1000
        )  # type: ignore
        df["CO₂ Flow, ml/min"] = df.get(
            "CO₂ Flow, ml/min", df.get("CO₂ Flow, l/min", np.nan) * 1000
        )  # type: ignore
...
class PiTable(_GeneralProcessTable):
    ...
    def process(self) -> pd.DataFrame:
        ...
        # Compute gas metrics
        df = self._compute_gas_metrics(
            df, "Outlet Flow N2 (%)", "Oulet Flow O2 (%)", "Oulet Flow CO2 (%)"
        )  # type: ignore
