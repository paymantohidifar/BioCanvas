...
PLOT_CONFIG: Dict[str, Dict[str, Any]] = {
    'qc': {                                                                     # Configuration for QC Plots
        ...
        'colors': sns.color_palette(palette='bright', as_cmap=True),             # matplotlib.colors.ListedColormap # type: ignore
        'markers': ['o', 's', 'D', '^', 'v', '>', '<', '+', 'p', '*', 'x', ''],  # Acceptable markers
        'marker_size': 9,                                                       # Marker size
    },
    'ol': {                                                                     # Configuration for KPI Overlap Plots
        'fig_size': (15, 6),
        'color_palette': sns.color_palette(palette='bright', as_cmap=True),             # matplotlib.colors.ListedColormap # type: ignore
        'marker_symbols': ['o', 's', 'D', '^', 'v', '>', '<', '+', 'p', '*', 'x', ''],  # Acceptable markers
        'marker_size': 9,                                                       # Marker size
    ...
    'cc': {                                                                     # Configuration for Condition Comparison plots
        ...
        'color_palette': sns.color_palette(palette='bright', n_colors=12),              # Per-condition line colors; multi-KPI bar colors
        'gray_palette': sns.color_palette(palette='gray', n_colors=3),           # Single-KPI aggregated bar colors (per replicate)
        'marker_symbols': ['o', 's', 'D', '^', 'v', '>', '<', '+', 'p', '*', 'x'], # Per-condition marker symbols
        'marker_size': 9,                                                        # Marker size
        'line_styles': ['-', '--', '.-'],                                         # Per-replicate line dash styles
    },
    'gc': {                                                                     # Configuration for Global Comparison plots
        ...
        'color_palette': sns.color_palette(palette='bright', n_colors=12),       # Default hue / multi-component bar colors (up to 12 categories)
        'gray_palette': sns.color_palette(palette='gray', n_colors=1),           # Single-component bar/box colors
        'line_markers': ['o', 's', 'D', '^', 'v', '>', '<', '+', 'p', '*'],      # Per-hue marker symbols for line plots
        'line_styles': ['-', '--', '-.', ':'],                                   # Per-hue line dash styles for line plots
    },
}
