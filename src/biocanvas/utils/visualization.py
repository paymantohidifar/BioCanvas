...
def generate_fig_description(
    description: dict[str, Any], file_path: str, fig_count: int
) -> None:
...
class QCPlotter:
    ...
    def make_plots(self) -> Optional[matplotlib.figure.Figure]:
        ...
        num_plots = sum(
            sum(content["col_exist"])
            for kpis in plot_properties.values()
            for content in kpis.values()
        )
        ...
        if subplots_nrows <= 1:
            ...
            fig_size = (
                subplots_ncols
                * styles.PLOT_CONFIG["qc"]["fig_size"]["multiple_rows"][0],
                subplots_nrows
                * styles.PLOT_CONFIG["qc"]["fig_size"]["multiple_rows"][1],
            )
    ...
    def __plot_lines(
        ...
        title: str,
        ylabel: str,
        x_lim: Tuple[Optional[float], Optional[float]],
        y_lim: Tuple[Optional[float], Optional[float]],
        marker_num: int,
    ) -> None:
...
class OLPlotter:
    ...
    def __init__(
        ...
        self._plot_tables: Dict[str, pd.DataFrame] = {
            k: v.copy() for k, v in plot_tables.items()
        }
        self._plot_properties: Dict[str, Dict[str, Dict[str, Any]]] = copy.deepcopy(
            plot_properties
        )
    ...
    def make_plots(
        ...
        try:
            ...
            kpi1_info: Dict[str, Any] = self._extract_information(
                panel1, group1, value1
            )
            kpi2_info: Dict[str, Any] = self._extract_information(
                panel2, group2, value2
            )
            ...
            logger.debug(
                "OL plot: kpi1_data=%r, kpi2_data=%r, kpi1_y=%r, kpi2_y=%r, kpi1_xlim=%r, kpi2_xlim=%r, kpi1_ylim=%r, kpi2_ylim=%r, kpi1_ylabel=%s, kpi2_ylabel=%s, use_marker1=%r, use_marker2=%r",
                kpi1_data,
                kpi2_data,
                kpi1_y,
                kpi2_y,
            ...
            fig, ax_handles = plt.subplots(
                1, 2, figsize=ol_style_properties["fig_size"]
            )  # type: ignore
            ...
            logger.error(
                "OLPlotter.make_plots: error plotting OL plot: %s", e, exc_info=True
            )
            raise PlottingError(f"OL Plotting failed.\n{e}") from e

    def _extract_information(
        self, kpi_panel: str, kpi_group: str, kpi_value: str
    ) -> Dict[str, Any]:
        """
        ...
        if kpi_panel not in plot_properties:
            logger.warning(
                "OLPlotter._extract_information: Panel '%s' not found in plot properties.",
                kpi_panel,
            )
            raise ValueError(f"Panel '{kpi_panel}' not found.")
        if kpi_group not in plot_properties[kpi_panel]:
            logger.warning(
                "OLPlotter._extract_information: KPI group '%s' not found in panel '%s'.",
                kpi_group,
                kpi_panel,
            )
        ...
        data: pd.DataFrame = self._plot_tables[kpi_panel][
            [("Tank", ""), ("Replicate", ""), ("Time (h)", "")] + resolved_y
        ]  # type: ignore
    ...
    def _plot_line(
        ...
        if (
            ...
            or y is None
            or ax is None
            or style_properties is None
        ):
            logger.warning(
                "OLPlotter._plot_line: Data, x, y, ax, and style_properties are required to plot lines."
            )
            raise ValueError("Missing required information to plot lines.")

        # Set plot style properties
        marker_symbols: List[str] = (
            style_properties["marker_symbols"]
            if marker
            else [""] * len(style_properties["marker_symbols"])
        )
    ...
    def _plot_scatter(
        ...
        if data is None or x is None or y is None or style_properties is None:
            logger.warning(
                "OLPlotter._plot_scatter: Data, x, y, and style_properties are required to plot scatter plot."
            )
...
class CCPlotter:
    ...
    def make_plots(
        ...
        try:
            ...
            logger.debug(
                "Extracted CC plot information: data=%s, y=%s, title=%s, ylabel=%s, xlim=%s, ylim=%s",
                plot_data.shape,
                kpi_y,
                kpi_title,
                kpi_ylabel,
    ...
    def _extract_information(
        ...
        if kpi_panel not in plot_properties:
            logger.warning(
                "CCPlotter._extract_information: Panel '%s' not found in plot properties.",
                kpi_panel,
            )
            raise ValueError(f"Panel '{kpi_panel}' not found.")
        if kpi_group not in plot_properties[kpi_panel]:
            logger.warning(
                "CCPlotter._extract_information: KPI group '%s' not found in panel '%s'.",
                kpi_group,
                kpi_panel,
            )
            raise ValueError(f"KPI group '{kpi_group}' not found.")
        ...
        if kpi_value == "All":
            ...
            y = [
                col
                for col in cols
                if col == kpi_value and (kpi_panel, col) in self._data.columns
            ]
        resolved_y = [(kpi_panel, col) for col in y]
        ...
        data: pd.DataFrame = self._data[
            [("Tank", ""), ("Time (h)", ""), ("Condition_num", ""), ("Replicate", "")]
            + resolved_y
        ]  # type: ignore
    ...
    def _plot_lines(
        ...
        if (
            ...
            logger.warning(
                "CCPlotter._plot_lines: Data, x, y, ax, and style_properties are required to plot lines."
            )
            raise ValueError("Missing required information to plot lines.")
        ...
        # Plot lines when there is only one y-axis column
        if len(y) == 1:
            data_grouped_by_condition: Iterable[tuple[int, pd.DataFrame]] = (
                data.groupby(("Condition_num", ""))
            )  # type: ignore
            if plot_type == "aggregate":
            ...
            logger.warning(
                "CCPlotter._plot_lines: Multiple KPI components for line plots are not supported. y=%r",
                y,
            )
            raise ValueError(
                "Multiple KPI components for line plots are not supported."
            )
    ...
    def _plot_bars(
        ...
        if data is None or x is None or y is None or style_properties is None:
            logger.warning(
                "CCPlotter._plot_bars: Data, x, y, and style_properties are required to plot bars."
            )
        ...
        if len(y) == 1:
            if plot_type == "aggregate":
                grps: Iterable[tuple[int, pd.DataFrame]] = data.groupby(
                    ("Condition_num", "")
                )  # type: ignore
            ...
            if plot_type == "aggregate":
                ...
                for idx, rep in enumerate(replicates):
                    ...
                    if not grp_rep.empty:
                        working_ax: matplotlib.axes.Axes = (
                            ax[idx] if len(replicates) > 1 else ax
                        )  # type: ignore
                        condition_nums_rep: Set[int] = set(
                            grp_rep[("Condition_num", "")].values
                        )  # type: ignore
...
class GCPlotter:
        ...
        try:
            ...
            if plot_type == "boxplot":
                self._box_plot(
                    ...
                    ylabel=kpi_ylabel,
                    y_lim=kpi_ylim,
                    style_properties=gc_style_properties,
                )
            # Plot lineplot if selected
            elif plot_type == "lineplot":
                line_data = (
                    plot_data_full_resolution
                    if not plot_data_full_resolution.empty
                    else plot_data
                )
                self._line_plot(
                    ax=ax,
                    ...
                    ylabel=kpi_ylabel,
                    y_lim=kpi_ylim,
                    use_markers=plot_data_full_resolution.empty,
                    style_properties=gc_style_properties,
                )
            ...
            result_data: pd.DataFrame = (
                line_data if plot_type == "lineplot" else plot_data
            )  # type: ignore
            ...
            raise PlottingError(
                "GC Plotting failed. Revise your selections and try again!"
            ) from None
    ...
    def _extract_information(
        ...
        if kpi_panel not in self._plot_properties:
            logger.warning(
                "GCPlotter._extract_information: panel %r not found in plot_properties.",
                kpi_panel,
            )
            raise ValueError(f"Panel '{kpi_panel}' not found in plot properties.")
        if kpi_group not in self._plot_properties[kpi_panel]:
            logger.warning(
                "GCPlotter._extract_information: kpi_value %r not found in panel %r.",
                kpi_value,
                kpi_panel,
            )
            raise ValueError(
                f"KPI group '{kpi_group}' not found in panel '{kpi_panel}'."
            )
        ...
        kpi_hue: Optional[str] = (
            second_grping_param if second_grping_param != "---" else None
        )
        ...
        if kpi_value == "All":
            ...
            y = [
                ...
                if col == kpi_value and col_exist[idx]
            ]
        resolved_y = [(kpi_panel, col) for col in y]
        ...
        data: pd.DataFrame = self._data[
            self._data[("Time (h)", "")].isin(sample_time_li)
        ]  # type: ignore
        if data.empty:
            logger.warning(
                "GCPlotter._extract_information: Data table is empty after time point filtering."
            )
            raise EmptyTableError("Data table is empty.")
        ...
        def _enrich_and_select(
            t: pd.DataFrame, add_first_grp: bool = True
        ) -> pd.DataFrame:
            ...
            if add_first_grp and first_grping_param != "Time (h)":
                t[first_grping_param] = self._master_meta_table.loc[
                    row_index, first_grping_param
                ].values  # type: ignore
            if second_grping_param not in ("---", "Time (h)"):
                t[second_grping_param] = self._master_meta_table.loc[
                    row_index, second_grping_param
                ].values  # type: ignore
        ...
        plot_data_full_resolution = (
            _enrich_and_select(data_full_resolution, add_first_grp=False)
            if not data_full_resolution.empty
            else pd.DataFrame()
        )
        ...
        if hue:
            ...
            if n_hues > len(color_palette):
                ...
                palette = sns.color_palette(
                    palette=style_properties["color_palette_name"], n_colors=n_hues
                )
            else:
