...
from biocanvas.utils import (
    auth,
    helpers,
    logging_config,
    visualization,
    stats as utils_stats,
    io as utils_io,
)
...
class App:
    ...
    def _wire_observers(self) -> None:
        ...
        # Process Data tab
        self.pd.project_dropdown.observe(
            self._pd_project_dropdown_on_change, names="value"
        )
        self.pd.passcode_box.observe(self._pd_passcode_on_change, names="value")
        self.pd.log_level_dropdown.observe(
            self._pd_log_level_dropdown_on_change, names="value"
        )
        ...
        self.pd.select_all_radio_button.observe(
            self._pd_select_all_radio_button_on_change, names="value"
        )
        ...
        self.ol.condition_dropdown.observe(
            self._ol_condition_dropdown_on_change, names="value"
        )
        self.ol.kpi1_group_dropdown.observe(
            self._ol_kpi1_group_dropdown_on_change, names="value"
        )
        self.ol.kpi2_group_dropdown.observe(
            self._ol_kpi2_group_dropdown_on_change, names="value"
        )
        ...
        self.cc.kpi_group_dropdown.observe(
            self._cc_kpi_group_dropdown_on_change, names="value"
        )
        self.cc.plot_button.on_click(self._handle_cc_plot_button)

        # Global Comparison tab
        self.gc.kpi_group_dropdown.observe(
            self._gc_kpi_group_dropdown_on_change, names="value"
        )
        self.gc.kpi_value_dropdown.observe(
            self._gc_kpi_value_dropdown_on_change, names="value"
        )
        self.gc.first_grping_param_dropdown.observe(
            self._gc_first_grping_param_dropdown_on_change, names="value"
        )
        ...
        self.gc.stacked_plot_checkbox.observe(
            self._gc_stacked_plot_checkbox_on_chnage, names="value"
        )
    ...
    def _pd_passcode_on_change(self, change: Dict[str, Any]) -> None:
        ...
        if auth.verify_passcode(stored_hash, entered):
            ...
            logger.debug(
                "Passcode accepted for project '%s'", self.pd.project_dropdown.value
            )
        else:
            self.pd.connect_button.disabled = True
    ...
    def _handle_pd_connect_button(self, _: Any) -> None:
        ...
        log_entries: List[helpers.LogEntry] = [
            (f"Connecting to project: {project_name}", "header")
        ]

        log_level = logging_config.LOG_LEVEL_MAP.get(
            self.pd.log_level_dropdown.value, logging.INFO
        )
    ...
    def _handle_pd_process_button(self, _: Any) -> None:
        ...
        exp_names = [
            cb.description for cb in self.pd.exp_checkbox_container.children if cb.value
        ]
        ...
        if not self.service.master_meta_table.empty:
            ...
            logging_config.display_log_html(
                self.pd.output_label,
                [("Master meta table is empty. No experiments processed.", "warning")],
            )
    ...
    def _activate_gc_tab(self) -> None:
        ...
        bench_kpi_li = [
            f"{panel} | {kpi}"
            for panel, kpis in self.service.bench_plot_properties.items()
            for kpi, content in kpis.items()
            if sum(content["col_exist"])
        ]
        process_kpi_li = [
            f"{panel} | {kpi}"
            for panel, kpis in self.service.process_plot_properties.items()
            for kpi, content in kpis.items()
            if sum(content["col_exist"])
        ]
        self._set_dropdown_options(
            self.gc.kpi_group_dropdown, bench_kpi_li + process_kpi_li
        )
        self._set_dropdown_options(
            self.gc.first_grping_param_dropdown,
            list(
    ...
    def _qc_exp_dropdown_on_change(self, change: Dict[str, Any]) -> None:
        ...
        conditions: List[str] = self.service.master_meta_table.loc[  # type: ignore
            self.qc.exp_dropdown.value, "Condition"
        ].unique()  # type: ignore
        self._set_dropdown_options(
            self.qc.condition_dropdown,
            [
                c
                for c in conditions
                if not c.lower().startswith("seed")  # type: ignore
            ],
        )
    ...
    def _handle_qc_plot_button(self, _: Any) -> None:
        ...
        try:
            ...
            logger.debug(
                "QCPlotter returned bench_fig=%s, process_fig=%s",
                bench_fig,
                process_fig,
            )
            ...
            if self.qc.savefig_checkbox.value and bench_fig:
                ...
                fig_description_file_path = os.path.join(
                    self.service.figs_dir, "plots_descriptions.json"
                )
    ...
    def _ol_exp_dropdown_on_change(self, change: Dict[str, Any]) -> None:
        """Updates the Condition dropdown when the experiment selection changes."""
        conditions: List[str] = self.service.master_meta_table.loc[  # type: ignore
            self.ol.exp_dropdown.value, "Condition"
        ].unique()
        self._set_dropdown_options(
            self.ol.condition_dropdown,
            [
                c
                for c in conditions
                if not c.lower().startswith("seed")  # type: ignore
        ...
        if panel != "Process":
            kpi_values: List[str] = [
                ...
                for idx, col in enumerate(
                    self.service.bench_plot_properties[panel][kpi]["cols"]
                )
            ...
            kpi_values = [
                col
                for idx, col in enumerate(
                    self.service.process_plot_properties[panel][kpi]["cols"]
                )
    ...
    def _handle_ol_plot_button(self, _: Any) -> None:
        ...
        try:
            ...
            kpi1_panel, kpi1_group = [
                s.strip() for s in self.ol.kpi1_group_dropdown.value.split("|")
            ]
            kpi2_panel, kpi2_group = [
                s.strip() for s in self.ol.kpi2_group_dropdown.value.split("|")
            ]
            kpi1_value = self.ol.kpi1_value_dropdown.value
            kpi2_value = self.ol.kpi2_value_dropdown.value
            reps: List[int] = [
                i for i in range(1, 4) if getattr(self.ol, f"rep{i}_checkbox").value
            ]
            ...
            bench_table: pd.DataFrame = self.service.update_bench_plot_properties(
                filter_keys
            )
            process_table: pd.DataFrame = self.service.update_process_plot_properties(
                filter_keys
            )
            ...
            if (
                kpi1_panel in self.service.bench_plot_properties
                and not bench_table.empty
            ):
                plot_properties[kpi1_panel] = self.service.bench_plot_properties[
                    kpi1_panel
                ]
                plot_tables[kpi1_panel] = bench_table
            elif (
                kpi1_panel in self.service.process_plot_properties
                and not process_table.empty
            ):
                plot_properties[kpi1_panel] = self.service.process_plot_properties[
                    kpi1_panel
                ]
                plot_tables[kpi1_panel] = process_table
            else:
                raise ValueError(
                    f"KPI1 panel {kpi1_panel} not found in bench or process plot properties"
                )
            if (
                kpi2_panel in self.service.bench_plot_properties
                and not bench_table.empty
            ):
                plot_properties[kpi2_panel] = self.service.bench_plot_properties[
                    kpi2_panel
                ]
                plot_tables[kpi2_panel] = bench_table
            elif (
                kpi2_panel in self.service.process_plot_properties
                and not process_table.empty
            ):
                plot_properties[kpi2_panel] = self.service.process_plot_properties[
                    kpi2_panel
                ]
                plot_tables[kpi2_panel] = process_table
            else:
                raise ValueError(
                    f"KPI2 panel {kpi2_panel} not found in bench or process plot properties"
                )

            # --- Generate OL plot and results ---
            ol_plotter: visualization.OLPlotter = visualization.OLPlotter(
                plot_tables, plot_properties
            )
                    ...
                    description={
                        ...
                        "Replicate": ", ".join(
                            [
                                str(i)
                                for i in range(1, 4)
                                if getattr(self.ol, f"rep{i}_checkbox").value
                            ]
                        ),
            ...
            logging_config.display_log_html(
                ...
                [
                    (
                        "Unexpected error occurred (see log for details). Revise your selections and try again!",
                        "warning",
                    )
                ],
            )
    ...
    def _cc_exp_dropdown_on_change(self, change: Dict[str, Any]) -> None:
        ...
        conditions: List[str] = self.service.master_meta_table.loc[  # type: ignore
            self.cc.exp_dropdown.value, "Condition"
        ].unique()
        conditions_info: List[Tuple[int, str]] = [
            (i + 1, c)
            for i, c in enumerate(conditions)  # type: ignore
        ...
        self._set_dropdown_options(
            self.cc.kpi_group_dropdown, bench_kpi_li + process_kpi_li
        )

        if (bench_table is not None) and (not bench_table.empty):  # type: ignore
            self._set_dropdown_options(
                self.cc.sample_time_dropdown,
                (
            ...
            if (process_table is not None) and (not process_table.empty):  # type: ignore
                ...
                self._set_dropdown_options(
                    self.cc.sample_time_dropdown,
                    list(range(0, max_process_time + 1)) + ["All"],
                )
            else:
    ...
    def _cc_kpi_group_dropdown_on_change(self, change: Dict[str, Any]) -> None:
        ...
        logger.debug(
            "CC plot: kpi_group_dropdown changed to %s",
            self.cc.kpi_group_dropdown.value,
        )
        ...
        logger.debug(
            "CC plot: kpi_value_dropdown options=%s", self.cc.kpi_value_dropdown.options
        )
        ...
        try:
            ...
            for n in range(0, len(children), 2):
                if children[n].value:
                    ...
                    condition_num = int(
                        children[n].description.split(":")[0].strip("Condition ")
                    )
                    replicate_li = [
                        i for i in range(1, 4) if children[n + 1].children[i].value
                    ]
                    condition_replicate_info[condition_desc] = (
                        condition_num,
                        replicate_li,
                    )
            ...
            plot_properties = (
                self.service.bench_plot_properties
                if is_bench
                else self.service.process_plot_properties
            )
            # Resolve data update function
            update_data = (
                self.service.update_bench_plot_properties
                if is_bench
                else self.service.update_process_plot_properties
            )
            active_conditions = {
            ...
            if ("Condition", "") not in data.columns and not data.empty:
                ...
                data[("Condition", "")] = self.service.master_meta_table.loc[
                    row_index, "Condition"
                ].values  # type: ignore
            ...
            concat_data: pd.DataFrame = pd.concat(
                df_li, axis="index", ignore_index=True
            )
            ...
            if (
                self.cc.verbose_stats_checkbox.value
                or self.cc.savefig_stats_checkbox.value
            ):
            ...
            if self.cc.verbose_stats_checkbox.value and cc_summary_stats is not None:
                with self.cc.verbose_stats_placeholder:
                    ...
                    display(
                        HTML(
                            "Table of summary statistics for each value of <b>Condition</b>:\n\n"
                        )
                    )
            ...
            if self.cc.savefig_stats_checkbox.value:
                ...
                visualization.generate_fig_description(
                    description={
                        ...
                        "Replicate": " | ".join(
                            [
                                ", ".join([str(el) for el in item[1]])
                                for item in condition_replicate_info.values()
                            ]
                        ),
                        "KPI Group": kpi_group,
                        "KPI Value": kpi_value,
            ...
            for filter_num, filter_key in enumerate(  # type: ignore
                ...
                self.gc.filter_accordion.children += (
                    widgets.SelectMultiple(
                        ...
                        value=[],
                        rows=5,
                        layout={"width": "auto", "padding": "0px 0px 0px 0px"},
                    ),
                )
        ...
        for idx, filter_key in enumerate(  # type: ignore
            ...
            if filter_key == "Exp":
                self.gc.filter_accordion.children[idx].options = list(
                    self.service.master_meta_table.index.get_level_values(
                        filter_key
                    ).unique()  # type: ignore
                )
    ...
    def _gc_kpi_group_dropdown_on_change(self, change: Dict[str, Any]) -> None:
        ...
        if panel != "Process":
            kpi_values: List[str] = [
                col
                for idx, col in enumerate(
                    self.service.bench_plot_properties[panel][kpi_group]["cols"]
                )
                if self.service.bench_plot_properties[panel][kpi_group]["col_exist"][
                    idx
                ]
            ]
        else:
            kpi_values = [
                col
                for idx, col in enumerate(
                    self.service.process_plot_properties[panel][kpi_group]["cols"]
                )
                if self.service.process_plot_properties[panel][kpi_group]["col_exist"][
                    idx
                ]
            ]
    ...
    def _gc_first_grping_param_dropdown_on_change(self, change: Dict[str, str]) -> None:
        ...
        local_group_keys: List[str] = list(
            self.service.config_module.GLOBAL_GROUP_KEYS  # type: ignore
        )
        local_group_keys.remove(change["new"])
        self._set_dropdown_options(
            self.gc.second_grping_param_dropdown, ["---"] + local_group_keys
        )
    ...
    def _handle_gc_plot_button(self, _: Any) -> None:
        ...
        try:
            ...
            filter_keys: Dict[str, Any] = {
                filter_key: (
                    "in",
                    f"{list(self.gc.filter_accordion.children[panel_num].value)}",
                )
                for panel_num, filter_key in enumerate(  # type: ignore
            ...
            sample_time_li: List[Any] = list(
                last_child.value if last_child.value else last_child.options
            )
            kpi_panel, kpi_group = [
                s.strip() for s in self.gc.kpi_group_dropdown.value.split("|")
            ]
            ...
            if (
                self.gc.verbose_stats_checkbox.value
                or self.gc.savefig_stats_checkbox.value
            ):
            ...
            if self.gc.verbose_stats_checkbox.value and gc_summary_stats is not None:
                ...
                with self.gc.verbose_stats_placeholder:
                    ...
                    display(
                        HTML(
                            f"Table of summary statistics for each value of "
            ...
            if self.gc.savefig_stats_checkbox.value:
                ...
                visualization.generate_fig_description(
                    description={
                        ...
                        "Filter": " | ".join(
                            [
                                f"<{key}>={', '.join([str(el) for el in val])}"
                                for key, val in filters_keys_for_figure.items()
                            ]
                        ),
                        "KPI Group": kpi_group,
                        "KPI Value": kpi_value,
                        "First Grouping Parameter": first_grping_param,
                        "Second Grouping Parameter": (
                            second_grping_param
    ...
    def _handle_pr_publish_report_button(self, _: Any) -> None:
        ...
        try:
            plots = sorted(
                [
                    el.split("/")[-1].rstrip(".png")
                    for el in glob(f"{self.service.figs_dir}/*.png")
                ],
                key=lambda el: (
                    int(re.search(r"\d+$", el).group()) if re.search(r"\d+$", el) else 0
                ),  # type: ignore
            )
            ...
            plot_description = {
                name: pd.DataFrame(
                    {"values": list(meta.values())}, index=list(meta.keys())
                )
        ...
        try:
            if os.path.exists(self.service.results_dir):
                ...
                with self.pr.output_label:
                    display(
                        HTML(
                            f"<b>{self.service.results_dir}</b> directory is zipped successfully."
                        )
                    )
            else:
                raise FileNotFoundError(
                    f"No <{self.service.results_dir}> directory found!"
                )
