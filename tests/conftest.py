    ...
    return {
        "Ferm": {
            "Biomass": {
                "cols": ["DCW (g/L)"],
                "col_exist": [1]
                if ("Ferm", "DCW (g/L)") in bench_table.columns
                else [0],
                "xlim": (None, None),
                "ylim": (None, None),
                "ylabel": "DCW (g/L)",
            ...
            "IS (%)": {
                ...
                "col_exist": [1]
                if ("Ferm", "% Insoluble Solids (g/g)") in bench_table.columns
                else [0],
                "xlim": (None, None),
                "ylim": (None, None),
    ...
    return {
        "Process": {
            "Temperature": {
                "cols": ["Temperature (°C)"],
                "col_exist": [1]
                if ("Process", "Temperature (°C)") in process_table.columns
                else [0],
                "xlim": (None, None),
                "ylim": (None, None),
...
def plot_properties_for_ol(
    ...
    return {
        "Ferm": bench_plot_properties["Ferm"],
        "Process": process_plot_properties["Process"],
    }
    ...
    return pd.DataFrame(
        {
            ...
            "Measured Added Feed (ml)": [
                68.0,
                "NA",
                "NA",
            ],  # Value added by rate x duration=67.2 ml; setting to 68.0 to ensure the correction factor is applied
    ...
    return pd.DataFrame(
        {
            ...
            "Time (h)": np.hstack(
                [np.tile([0.0, 24.0, 72.0, 100.0], 2), np.array([0.0, 24.0])]
            ),
