...
class TestAugmentTables:
    ...
    def test_meta_table_unchanged(
        ...
        pd.testing.assert_frame_equal(
            augmented.augmented_meta_table, meta_table_for_augment
        )
    ...
    def test_empty_process_time_span_matches_eft(
        ...
        tank_col = ("Tank", "") if isinstance(proc.columns, pd.MultiIndex) else "Tank"
        time_col = (
            ("Time (h)", "") if isinstance(proc.columns, pd.MultiIndex) else "Time (h)"
        )
    ...
    def test_process_augment_feed_totals_are_finite_for_ferm_tanks(
        ...
        for col_name in [
            ...
            assert not col.isna().any(), (
                f"NaN values found in '{col_name}' for ferm tanks"
            )
    ...
    def test_empty_bench_stays_empty(
        ...
        assert (
            ...
            not in augmented.output_messages
        )
    ...
    def test_bench_seed_tank_broth_vol_equals_initial(
        ...
        expected_vol = meta_table_for_augment.set_index("Tank").loc[
            "S1", "Initial Broth Vol (ml)"
        ]
        s1_rows = augmented.augmented_bench_table[
            augmented.augmented_bench_table["Tank"] == "S1"
        ]
        assert s1_rows["Total Broth Vol (ml)"].tolist() == pytest.approx(
            [expected_vol] * len(s1_rows)
        )
    ...
    def test_try_kpi_titer_corrected_for_insoluble_solids(
        ...
        assert s1_t24[("Lcuv", "TestProduct Titer (g/L)")] == pytest.approx(
            expected_titer
        )
    ...
    def test_try_kpi_drops_raw_titer_column(
        ...
        assert (
            "Lcuv",
            "TestProduct (g/L)",
        ) not in augmented.augmented_bench_table.columns
    ...
    def test_path_a_pumped_vol_equals_pump_total_column(
        ...
        """
        augmented = self._make_instance(
            meta_exact_for_augment,
            pd.DataFrame(),
            process_exact_for_augment,
    ...
    def test_try_kpi_missing_product_column_is_gracefully_skipped(
        ...
        assert any(
            "Successfully augmented Benchling result table" in m
