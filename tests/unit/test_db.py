...
class TestImportMetaTable:
    ...
    @staticmethod
    def _import(db_conn: sqlite3.Connection, meta_table_for_augment: pd.DataFrame) -> int:
    ...
    def test_round_trip(self, db_conn: sqlite3.Connection, meta_table_for_augment: pd.DataFrame):
    ...
    def test_feed_profiles_populated_only_for_real_sources(
        ...
        l1_feed_types = {
            ...
            for r in db_conn.execute(
                'SELECT feed_type FROM tank_feed_profile WHERE tank_meta_id = ?', (l1_id,)
            ).fetchall()
        }
...
class TestImportBenchlingTable:
    ...
    @staticmethod
    def _import(db_conn: sqlite3.Connection, raw_bench_table_for_augment: pd.DataFrame) -> int:
        project_id = importer.get_or_create_project(db_conn, 'helix')
        experiment_id = importer.get_or_create_experiment(db_conn, project_id, 'E001')
        importer.import_benchling_table(db_conn, experiment_id, raw_bench_table_for_augment)
        return experiment_id
    ...
    def test_melts_to_long(
        ...
        sample_count = db_conn.execute(
            'SELECT COUNT(*) FROM benchling_sample WHERE experiment_id = ?', (experiment_id,)
        ).fetchone()[0]
    ...
    def test_reconstructs_tank_and_value_via_join(
        ...
        expected = raw_bench_table_for_augment[
            (raw_bench_table_for_augment['Tank'] == 'L1') & (raw_bench_table_for_augment['Time (h)'] == 0.0)
...
class TestImportProcessTable:
    ...
    @staticmethod
    def _import(db_conn: sqlite3.Connection, raw_process_table_for_augment: pd.DataFrame) -> int:
        project_id = importer.get_or_create_project(db_conn, 'helix')
        experiment_id = importer.get_or_create_experiment(db_conn, project_id, 'E001')
        importer.import_process_table(db_conn, experiment_id, raw_process_table_for_augment, 'Eve')
        return experiment_id
    ...
    def test_melts_to_long(
        ...
        assert measurement_count == len(raw_process_table_for_augment) * 2  # 2 pump-volume channels
    ...
    def test_reimport_does_not_duplicate(
        ...
        file_count = db_conn.execute(
            'SELECT COUNT(*) FROM process_file WHERE experiment_id = ?', (experiment_id,)
        ).fetchone()[0]
        assert file_count == 3
...
class TestImportExperiment:
    ...
    def test_multi_project_isolation(
        ...
        helix_experiment_id = importer.import_experiment(
            db_conn, 'helix', 'E001',
            meta_table_for_augment, raw_bench_table_for_augment, raw_process_table_for_augment, 'Eve',
        )
        ...
        helix_tank_count = db_conn.execute(
            'SELECT COUNT(*) FROM tank_meta WHERE experiment_id = ?', (helix_experiment_id,)
        ).fetchone()[0]
        spore_tank_count = db_conn.execute(
            'SELECT COUNT(*) FROM tank_meta WHERE experiment_id = ?', (spore_experiment_id,)
        ).fetchone()[0]
        assert helix_tank_count == 3
        assert spore_tank_count == 3

        total_experiments = db_conn.execute('SELECT COUNT(*) FROM experiment').fetchone()[0]
