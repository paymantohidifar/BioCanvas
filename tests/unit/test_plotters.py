...
import biocanvas.helix.config as config
...
class TestOLPlotterInit:
    ...
    def test_stores_deep_copies(
        ...
        assert plotter._plot_tables['Process'].loc[0, ("Process", "Temperature (°C)")] != 999.0
...
class TestOLPlotterMakePlots:
    ...
    def test_returns_none_when_first_kpi_missing(
        ...
        with pytest.raises(visualization.PlottingError, match="KPI group 'Biomass' not found"):
            plotter.make_plots("Ferm", "Biomass", "DCW (g/L)", "Process", "Temperature", "Temperature (°C)", "Aggregate")
    ...
    def test_returns_fig_and_data_when_both_kpis_valid(
        ...
        mocker.patch("biocanvas.utils.visualization.plt.subplots", return_value=(fig, axes))
        mocker.patch("biocanvas.utils.visualization.plt.tight_layout")
        mocker.patch("biocanvas.utils.visualization.plt.show")
        mocker.patch("biocanvas.utils.visualization.display")
        plotter = visualization.OLPlotter(
            {'Ferm': bench_table_for_ol, 'Process': process_table_for_ol},
            plot_properties_for_ol,
        )
        ...
        # Act
        result = plotter.make_plots("Ferm", "Biomass", "DCW (g/L)", "Process", "Temperature", "Temperature (°C)", "Aggregate")
...
class TestCCPlotterInit:
    ...
    def test_stores_deep_copy_of_plot_properties(
        ...
        assert plotter._plot_properties['Ferm']['Biomass']['col_exist'] == original_col_exist
...
class TestCCPlotterExtractInformation:
    ...
    def test_raises_value_error_for_unknown_panel(self, plot_properties_for_cc):
        ...
        with pytest.raises(ValueError, match="Panel 'UnknownPanel' not found"):
            plotter._extract_information(kpi_panel="UnknownPanel", kpi_group="Biomass", kpi_value="DCW (g/L)")
    ...
    def test_raises_value_error_for_unknown_kpi_group(self, plot_properties_for_cc):
        ...
        with pytest.raises(ValueError, match="KPI group 'UnknownGroup' not found"):
            plotter._extract_information(kpi_panel="Ferm", kpi_group="UnknownGroup", kpi_value="DCW (g/L)")

    def test_col_exist_is_not_used_in_column_resolution(self, plot_properties_for_cc, update_table_for_cc):
        ...
        result = plotter._extract_information(kpi_panel="Ferm", kpi_group="Biomass", kpi_value="DCW (g/L)")
        assert result['y'] == [("Ferm", "DCW (g/L)")]

    def test_returns_empty_y_when_kpi_value_not_in_data_columns(self, plot_properties_for_cc, update_table_for_cc):
        plotter = self._make_plotter(plot_properties_for_cc, update_table_for_cc.copy())

        result = plotter._extract_information(kpi_panel="Ferm", kpi_group="Biomass", kpi_value="UnknownMetric")
        assert result['y'] == []

    def test_raises_empty_table_exception_when_data_is_empty(self, plot_properties_for_cc, update_table_for_cc):
        # Pass a table with correct columns but zero rows to trigger EmptyTableError
        plotter = self._make_plotter(plot_properties_for_cc, update_table_for_cc.head(0))

        with pytest.raises(helpers.EmptyTableError, match="Data table is empty"):
            plotter._extract_information(kpi_panel="Ferm", kpi_group="Biomass", kpi_value="DCW (g/L)")

    def test_assembles_plot_data_for_single_kpi_value(self, plot_properties_for_cc, update_table_for_cc):
        plotter = self._make_plotter(plot_properties_for_cc, update_table_for_cc.copy())
    ...
    def test_assembles_kpi_y_list_for_all_value(self, plot_properties_for_cc, update_table_for_cc):
        plotter = self._make_plotter(plot_properties_for_cc, update_table_for_cc.copy())

        result = plotter._extract_information(kpi_panel="Sugar", kpi_group="Growth Substrate", kpi_value="All")

        assert result['y'] == [("Sugar", "Glucose (g/L)"), ("Sugar", "Fructose (g/L)"), ("Sugar", "Sucrose (g/L)")]
        assert not result['data'].empty

    def test_returns_all_timepoints_in_data(self, plot_properties_for_cc, update_table_for_cc):
...
class TestCCPlotterMakePlots:
    ...
    def test_returns_none_when_assembled_data_is_empty(self, plot_properties_for_cc, update_table_for_cc):
        ...
        with pytest.raises(visualization.PlottingError, match="Data table is empty"):
            plotter.make_plots(kpi_panel="Ferm", kpi_group="Biomass", kpi_value="DCW (g/L)", timepoint="All", plot_type="aggregate")

    def test_returns_fig_for_line_plot_all_timepoints(self, plot_properties_for_cc, update_table_for_cc, mocker):
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt

        fig, ax = plt.subplots(1, 1, figsize=(4.0, 3.0))
        mocker.patch("biocanvas.utils.visualization.plt.subplots", return_value=(fig, ax))
        mocker.patch("biocanvas.utils.visualization.plt.show")

        plotter = self._make_plotter(plot_properties_for_cc, update_table_for_cc.copy())

        result = plotter.make_plots(kpi_panel="Ferm", kpi_group="Biomass", kpi_value="DCW (g/L)", timepoint="All", plot_type="aggregate")
    ...
    def test_returns_fig_for_bar_plot_single_timepoint(self, plot_properties_for_cc, update_table_for_cc, mocker):
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
        ...
        result = plotter.make_plots(kpi_panel="Ferm", kpi_group="Biomass", kpi_value="DCW (g/L)", timepoint=1.0, plot_type="aggregate")
...
class TestCCPlotterDrawLines:
    ...
    def test_raises_value_error_when_data_x_y_are_none(self):
        with pytest.raises(ValueError, match="Missing required information to plot lines."):
...
class TestGCPlotterExtractInformation:
    ...
    def _make_plotter(self, plot_properties, data, master_meta_table) -> visualization.GCPlotter:
    ...
    def test_raises_value_error_for_unknown_panel(self, plot_properties_for_gc, master_meta_table_for_gc):
        plotter = self._make_plotter(plot_properties_for_gc, pd.DataFrame(), master_meta_table_for_gc)

        with pytest.raises(ValueError, match="Panel 'Unknown' not found in plot properties."):
            plotter._extract_information(
                kpi_panel='Unknown', kpi_group='Biomass', kpi_value='DCW (g/L)',
                first_grping_param='Condition', second_grping_param='---', sample_time_li=[24.0],
            )

    def test_raises_value_error_for_unknown_kpi_group(self, plot_properties_for_gc, master_meta_table_for_gc):
        plotter = self._make_plotter(plot_properties_for_gc, pd.DataFrame(), master_meta_table_for_gc)

        with pytest.raises(ValueError, match="KPI group 'Unknown' not found in panel 'Ferm'."):
            plotter._extract_information(
                kpi_panel='Ferm', kpi_group='Unknown', kpi_value='DCW (g/L)',
                first_grping_param='Condition', second_grping_param='---', sample_time_li=[24.0],
            )

    def test_raises_empty_table_exception_when_update_table_returns_empty(
        self, plot_properties_for_gc, update_table_for_gc, master_meta_table_for_gc,
    ):
        # Table with correct columns but zero rows triggers EmptyTableError
        plotter = self._make_plotter(plot_properties_for_gc, update_table_for_gc.head(0), master_meta_table_for_gc)
    ...
    def test_raises_empty_table_exception_when_sample_time_filter_empties_table(
        ...
        plotter = self._make_plotter(plot_properties_for_gc, update_table_for_gc.copy(), master_meta_table_for_gc)

        with pytest.raises(helpers.EmptyTableError):
            plotter._extract_information(
                kpi_panel='Ferm', kpi_group='Biomass', kpi_value='DCW (g/L)',
                first_grping_param='Condition', second_grping_param='---', sample_time_li=[999.0],
            )
    ...
    def test_returns_correct_dict_keys_for_bench_selection(
        ...
        for key in ('data', 'data_full_resolution', 'y', 'title', 'ylabel', 'xlim', 'ylim', 'hue'):
    ...
    def test_data_full_resolution_is_empty_for_bench_panel(
        ...
        result = plotter._extract_information(
            kpi_panel='Ferm', kpi_group='Biomass', kpi_value='DCW (g/L)',
            first_grping_param='Time (h)', second_grping_param='---', sample_time_li=[24.0],
        )
    ...
    def test_data_full_resolution_is_non_empty_for_process_time_selection(
        ...
        result = plotter._extract_information(
            kpi_panel='Process', kpi_group='pH Profile', kpi_value='pH',
            first_grping_param='Time (h)', second_grping_param='---', sample_time_li=[24.0],
        )
...
class TestGCPlotterMakePlot:
    ...
    def test_returns_gc_plot_result_for_boxplot(
        self, plot_properties_for_gc, update_table_for_gc, master_meta_table_for_gc, mocker,
    ):
        import matplotlib
        matplotlib.use('Agg')
        import matplotlib.pyplot as plt
        ...
        result = plotter.make_plot(
            kpi_panel='Ferm', kpi_group='Biomass', kpi_value='DCW (g/L)',
            first_grping_param='Condition', second_grping_param='---',
            sample_time_li=[24.0], plot_type='boxplot',
        )
    ...
    def test_returns_gc_plot_result_for_barplot(
        ...
        result = plotter.make_plot(
            kpi_panel='Ferm', kpi_group='Biomass', kpi_value='DCW (g/L)',
            first_grping_param='Condition', second_grping_param='---',
            sample_time_li=[24.0], plot_type='barplot',
        )
    ...
    def test_returns_gc_plot_result_for_lineplot_process(
        ...
        result = plotter.make_plot(
            kpi_panel='Process', kpi_group='pH Profile', kpi_value='pH',
            first_grping_param='Time (h)', second_grping_param='---',
            sample_time_li=[24.0], plot_type='lineplot',
        )
