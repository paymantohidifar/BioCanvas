...
class TestFerm:
    ...
    def test_aggregates_mean_and_std(self):
        ...
        dcw = [10.0, 14.0]
        assert result.loc["M1-R1-T24h", ("Ferm", "DCW (g/L)")] == pytest.approx(
            np.mean(dcw)
        )
        assert result.loc["M1-R1-T24h", ("Ferm", "DCW_std (g/L)")] == pytest.approx(
            np.std(dcw, ddof=1)
        )
...
class TestBiochem:
    ...
    def test_renames_cellobiohydrolase_column(self):
        ...
        result = _panel_from_df(
            benchling.Biochem, df, arcname="exp.Biochem.csv"
        ).process()
